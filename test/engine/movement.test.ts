...
describe('B5 — movement', () => {
  ...
  it('contact mode: spends OMP against terrain costs hex by hex', () => {
    ...
    run(truth, 1);
    // budget 4: woods 2 + woods 2 = 4 spent; clear hex (1) unaffordable
    expect(posOf(truth, 'm1')).toEqual(gp(7, 5));

    run(truth, 1);
    expect(posOf(truth, 'm1')).toEqual(gp(8, 5)); // arrives
  ...
  it('pulse mode: OMP×10 on roads, OMP×5 cross-country (core §2.3)', () => {
    const roadHexes = Array.from({ length: 25 }, (_, i) =>
      ({ q: i, r: 5, infra: ['ROAD' as const] }));
    const truth = baseTruth('MOVE-SEED', roadHexes, 40);
    truth.clockMode = 'PULSE';
    // road mover: omp 2 ⇒ 20 hexes per pulse
    const fr = addMechFormation(truth, { id: 'road', sideId: 'blue', pos: gp(0, 5), omp: 2 });
    activate(truth, moveOrder('or', fr, 'MOVE',
      Array.from({ length: 24 }, (_, i) => gp(i + 1, 5))));
    // cross-country mover: omp 2 ⇒ 10 hexes per pulse
    const fx = addMechFormation(truth, { id: 'xc', sideId: 'blue', pos: gp(0, 10), omp: 2 });
    activate(truth, moveOrder('ox', fx, 'MOVE',
      Array.from({ length: 24 }, (_, i) => gp(i + 1, 10))));

    run(truth, 10); // one pulse
    expect(posOf(truth, 'road')).toEqual(gp(20, 5));
    expect(posOf(truth, 'xc')).toEqual(gp(10, 10));
  });

  it('forced march: ×1.5 speed and RDY −1 per pulse marched', () => {
    const truth = baseTruth('MOVE-SEED', [], 40);
    truth.clockMode = 'PULSE';
    const f = addMechFormation(truth, { id: 'fm', sideId: 'blue', pos: gp(0, 10), omp: 2 });
    activate(truth, moveOrder('of', f, 'FORCED_MARCH',
      Array.from({ length: 30 }, (_, i) => gp(i + 1, 10))));

    run(truth, 10);
    expect(posOf(truth, 'fm')).toEqual(gp(15, 10)); // 2 × 5 × 1.5
  ...
  it('cautious move: half speed (D-006)', () => {
    const truth = baseTruth('MOVE-SEED', [], 40);
    truth.clockMode = 'PULSE';
    const f = addMechFormation(truth, { id: 'cm', sideId: 'blue', pos: gp(0, 10), omp: 2 });
    activate(truth, moveOrder('oc', f, 'MOVE_CAUTIOUS',
      Array.from({ length: 20 }, (_, i) => gp(i + 1, 10))));
    run(truth, 10);
    expect(posOf(truth, 'cm')).toEqual(gp(5, 10)); // 2 × 5 × 0.5
  ...
  it('fractional progress carries across steps (no movement lost to rounding)', () => {
    const truth = baseTruth('MOVE-SEED', [], 40);
    truth.clockMode = 'CONTACT';
    const f = addMechFormation(truth, { id: 's1', sideId: 'blue', pos: gp(0, 3), omp: 1 },
      4, { walkOrCruise: 1 });
    // woods everywhere on the path: cost 2/hex with omp 1 ⇒ one hex per 2 contact turns
